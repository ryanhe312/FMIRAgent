...
class Model(nn.Module):
    ...
    def forward(self, x, idx_scale):
        ...
        if self.chop and not self.training and self.args.test_only:
            if 'proj' in self.args.model:
                ...
                result = self.forward_chop(x, min_size=60000)
        else:
            result = self.model(x)
    ...
    def load(self, apath, modelpath='.', resume=-1):
        ...
        if resume == -1:
            print('Load Model from ', os.path.join(apath, 'model/model_latest.pt'))
            ...
            m = os.path.join(apath, 'model/model_best.pt')
            print('Load Model from ', m)
            state_dict = {k.replace('module.', ''): v for k, v in torch.load(m, **kwargs).items()}
            self.model.load_state_dict(state_dict, strict=True)
        elif resume < -2:
            m = os.path.join(apath, 'model/model_best%d.pt' % -resume)
            print('Load Model from ', m)
            ...
            if ('2stg_enlcn' in self.args.model) or ('2stg_proj_care' in self.args.model):
                ...
                state_dict = {k.replace('module.', ''): v for k, v in torch.load(modelpath, **kwargs).items()}
                self.get_model().load_state_dict(state_dict, strict=True)
        elif (resume > 0) and os.path.exists(os.path.join(apath, 'model/model_{}.pt'.format(resume))):
            print('Load Model from ', os.path.join(apath, 'model/model_{}.pt'.format(resume)))
